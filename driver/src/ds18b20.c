...
/**************************************************************************/
char Temperature[5]="00.0";

void delay_nus(vu32 nCount)
{
  unsigned int TIMCounter = nCount;
...
void DS18B20_init(void)
{
    ...
    GPIO_Init(DS18B20_PORT, &GPIO_InitStructure);
		...
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
		TIM_TimeBaseStructure.TIM_Period = 1;                 //自动装载
		TIM_TimeBaseStructure.TIM_Prescaler = 100;//72;       //72M分频率到1MHz
		TIM_TimeBaseStructure.TIM_ClockDivision = 0;  
		TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Down;  //向下计数
		TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);	
}
...
unsigned int DS18B20_Read(void)                   //读取温度值
{  
    u16 res;
    init_1820();  
